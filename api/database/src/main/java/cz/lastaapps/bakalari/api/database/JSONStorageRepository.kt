...
class JSONStorageRepository(val database: APIBase) {
    ...
    abstract class JSONStorageDao {
        ...
        @Query("SELECT * FROM ${APIBaseKeys.JSON_STORAGE} WHERE id=:key")
        abstract fun getPair(key: String): JSONPair?

        @Query("SELECT data FROM ${APIBaseKeys.JSON_STORAGE} WHERE id LIKE :key")
        ...
        @Query("DELETE FROM ${APIBaseKeys.JSON_STORAGE}")
        abstract fun deleteAll()
    }

    @Entity(tableName = APIBaseKeys.JSON_STORAGE)
