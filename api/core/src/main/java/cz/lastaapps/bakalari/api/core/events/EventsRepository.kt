...
import cz.lastaapps.bakalari.api.core.database.RefreshingRepo
import cz.lastaapps.bakalari.api.core.events.holders.Event
import cz.lastaapps.bakalari.api.core.events.holders.EventHolderWithLists
...
class EventsRepository(database: APIBase) : RefreshingRepo<EventList, JSONList>(TAG, database) {
    ...
    fun getEvents() = dao.getEvents().distinctUntilChanged()
        .map { list -> cz.lastaapps.bakalari.api.core.events.holders.EventList(list.map { it.toEvent() }) }
    ...
    private fun combineEvents(pairs: List<Pair<EventType, EventList>>): EventList {
        ...
        val combined = cz.lastaapps.bakalari.api.core.events.holders.EventList()
