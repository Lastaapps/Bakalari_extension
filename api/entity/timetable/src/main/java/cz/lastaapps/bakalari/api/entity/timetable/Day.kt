...
@Parcelize
@Entity(tableName = APIBaseKeys.TIMETABLE_DAY)
data class Day(
    val dayOfWeek: Int,
    @PrimaryKey
    ...
    fun getHolidayDescription(context: Context, emoji: Boolean = false): String {
        return if (description != "") {
            if (!emoji) description else descriptionEmoji
        } else {
            if (dayType == CELEBRATION)
                context.getString(R.string.timetable_celebration)
            else
                context.getString(R.string.timetable_holiday)
        }
    }
