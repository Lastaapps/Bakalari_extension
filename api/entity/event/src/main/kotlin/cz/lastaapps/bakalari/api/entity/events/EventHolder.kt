...
@Entity(tableName = APIBaseKeys.EVENTS)
...
@Entity(
    tableName = APIBaseKeys.EVENTS_CLASSES_DATA,
    primaryKeys = ["data_id"],
    indices = [Index(value = ["data_id"])]
)
data class EventClassData(@Embedded(prefix = "data_") val data: SimpleData)

@Entity(primaryKeys = ["id", "data_id"], tableName = APIBaseKeys.EVENTS_CLASSES_RELATIONS)
data class EventClassRelation(
    @ColumnInfo(index = true) val id: String,
    @ColumnInfo(name = "data_id", index = true) val dataId: String
)

@Entity(primaryKeys = ["id", "data_id"], tableName = APIBaseKeys.EVENTS_TEACHES)
data class EventTeacherRelation(
    @ColumnInfo(index = true) val id: String,
    @ColumnInfo(name = "data_id", index = true) val dataId: String
)

@Entity(primaryKeys = ["id", "data_id"], tableName = APIBaseKeys.EVENTS_ROOMS)
data class EventRoomRelation(
    @ColumnInfo(index = true) val id: String,
    @ColumnInfo(name = "data_id", index = true) val dataId: String
)

@Entity(primaryKeys = ["id", "data_id"], tableName = APIBaseKeys.EVENTS_STUDENTS)
