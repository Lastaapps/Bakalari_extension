...
package cz.lastaapps.bakalari.api.dao.mark
...
abstract class MarksDao {
    ...
    @Transaction
    @Query("SELECT * FROM ${APIBaseKeys.MARK_SUBJECT} ORDER BY name COLLATE LOCALIZED")
    abstract fun getAllPairs(): Flow<List<MarksPair>>

    @Transaction
    @Query("SELECT * FROM ${APIBaseKeys.MARK_SUBJECT} WHERE id=:subjectId LIMIT 1")
    abstract fun getPair(subjectId: String): Flow<MarksPair?>


    @Query("SELECT * FROM (SELECT * FROM ${APIBaseKeys.MARKS} WHERE editDate > :date OR date > :date ORDER BY date DESC) UNION SELECT * FROM ${APIBaseKeys.MARKS} WHERE editDate <= :date AND date <= :date ORDER BY date DESC")
    abstract fun getAllMarks(date: ZonedDateTime): Flow<List<Mark>>

    @Query("SELECT * FROM (SELECT * FROM ${APIBaseKeys.MARKS} WHERE (editDate > :date OR date > :date) AND subjectId=:subjectId ORDER BY date DESC) UNION SELECT * FROM ${APIBaseKeys.MARKS} WHERE editDate <= :date AND date <= :date AND subjectId=:subjectId ORDER BY date DESC")
    abstract fun getMarks(subjectId: String, date: ZonedDateTime): Flow<List<Mark>>

    @Query("SELECT * FROM ${APIBaseKeys.MARKS} WHERE editDate > :date OR date > :date ORDER BY date DESC")
    abstract fun getNewMarks(date: ZonedDateTime): Flow<List<Mark>>

    @Query("SELECT * FROM ${APIBaseKeys.MARKS} WHERE id=:markId LIMIT 1")
    ...
    @Query("DELETE FROM ${APIBaseKeys.MARKS}")
    ...
    @Query("DELETE FROM ${APIBaseKeys.MARK_SUBJECT}")
