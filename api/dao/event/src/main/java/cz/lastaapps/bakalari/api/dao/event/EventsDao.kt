...
package cz.lastaapps.bakalari.api.dao.event

import androidx.room.*
import cz.lastaapps.bakalari.api.entity.core.APIBaseKeys
import cz.lastaapps.bakalari.api.entity.core.RoomData
import cz.lastaapps.bakalari.api.entity.core.StudentData
...
abstract class EventsDao {
    ...
    //get part
    @Transaction
    @Query("SELECT * FROM ${APIBaseKeys.EVENTS} ORDER BY eventStart DESC")
    abstract fun getEvents(): Flow<List<EventHolderWithLists>>

    @Transaction
    @Query("SELECT * FROM ${APIBaseKeys.EVENTS} WHERE id=:id LIMIT 1")
    ...
    suspend fun insert(list: List<EventHolderWithLists>) {
        ...
        for (item in list) {
            ...
            classRelList.addAll(item.classes.map {
                EventClassRelation(
                    item.holder.id,
                    it.data.id
                )
            ...
            roomRelList.addAll(item.rooms.map {
                EventRoomRelation(
                    item.holder.id,
                    it.data.id
                )
    ...
    @Query("DELETE FROM ${APIBaseKeys.EVENTS}")
    protected abstract suspend fun deleteAllEvents()

    @Query("DELETE FROM ${APIBaseKeys.EVENTS_TIMES}")
    protected abstract suspend fun deleteAllTimesData()

    @Query("DELETE FROM ${APIBaseKeys.EVENTS_CLASSES_DATA}")
    protected abstract suspend fun deleteAllClassesData()

    @Query("DELETE FROM ${APIBaseKeys.EVENTS_CLASSES_RELATIONS}")
    protected abstract suspend fun deleteAllClassesRelations()

    @Query("DELETE FROM ${APIBaseKeys.EVENTS_TEACHES}")
    protected abstract suspend fun deleteAllTeachers()

    @Query("DELETE FROM ${APIBaseKeys.EVENTS_ROOMS}")
    protected abstract suspend fun deleteAllRooms()

    @Query("DELETE FROM ${APIBaseKeys.EVENTS_STUDENTS}")
