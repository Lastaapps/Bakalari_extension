...
import cz.lastaapps.bakalariextension.api.homework.data.HomeworkHolderWithData
...
abstract class HomeworkDao {
    ...
    //get
    @Transaction
    @Query("SELECT * FROM ${APIBase.HOMEWORK} ORDER BY dateEnd DESC, dateStart DESC")
    abstract fun getHomeworkList(): Flow<List<HomeworkHolderWithData>>

    @Transaction
    @Query("SELECT * FROM ${APIBase.HOMEWORK} WHERE dateEnd >= :date OR done == 0 ORDER BY dateEnd DESC, dateStart DESC")
    abstract fun getCurrentHomeworkList(date: ZonedDateTime): Flow<List<HomeworkHolderWithData>>

    @Transaction
    @Query("SELECT * FROM ${APIBase.HOMEWORK} WHERE dateEnd < :date AND done != 0 ORDER BY dateEnd DESC, dateStart DESC")
    abstract fun getOldHomeworkList(date: ZonedDateTime): Flow<List<HomeworkHolderWithData>>

    @Transaction
    @Query("SELECT * FROM ${APIBase.HOMEWORK} WHERE subject_id=:subjectId ORDER BY dateEnd DESC, dateStart DESC")
    abstract suspend fun getHomeworkListForSubject(subjectId: String): List<HomeworkHolderWithData>

    @Transaction
    @Query("SELECT * FROM ${APIBase.HOMEWORK} WHERE id=:id LIMIT 1")
    abstract suspend fun getHomework(id: String): HomeworkHolderWithData?

    @Query("SELECT id FROM ${APIBase.HOMEWORK} WHERE dateEnd >= :date OR done == 0 ORDER BY dateEnd DESC, dateStart DESC")
    ...
    @Query("DELETE FROM ${APIBase.HOMEWORK}")
