...
import cz.lastaapps.bakalari.app.api.timetable.data.Lesson
...
class NotificationContent(val context: Context) {
    ...
    private fun lessonNextLesson(
        ...
        //before lesson starts
        actions[begin] = arrayOf(
            "${pattern.begin} ${"<b>" + week.rooms.getById(lesson.roomId)?.name + "</b>"} - ${week.subjects.getById(lesson.subjectId)?.name}",
            "${week.teachers.getById(lesson.teacherId)?.name}${if (lesson.theme != "") " ${lesson.theme}" else ""}, $groupStr ${week.groups.getByIds(lesson.groupIds)?.shortcut}"
        )

        //during lesson
        actions[end - 10 * 60] = arrayOf(
            "${"<b>" + week.rooms.getById(lesson.roomId)?.name + "</b>"} - ${week.subjects.getById(lesson.subjectId)?.name}",
        ...
        actions[end] = arrayOf(
            "$nextStr: <b>${week.rooms.getById(nextLesson.roomId)?.name}</b> - ${week.subjects.getById(nextLesson.subjectId)?.name}",
    ...
    private fun freeNextLesson(
        ...
        actions[end - 10 * 60] = arrayOf(
            ...
            "$nextStr: ${nextPattern.begin} ${week.rooms.getById(nextLesson.roomId)?.name} - ${week.subjects.getById(nextLesson.subjectId)?.name}"
        )
    ...
    private fun absenceNextLesson(
        ...
        actions[end - 10 * 60] = arrayOf(
            "${lesson.change?.typeShortcut} ${lesson.change?.typeName}",
            "${pattern.begin} -: ${pattern.end}, $nextStr: ${nextPattern.begin} ${week.rooms.getById(nextLesson.roomId)?.name} - ${week.subjects.getById(nextLesson.subjectId)?.name}"
        )
