...
object CheckInternet {
     ...
     * @param url (API) url to check for activity, if null, google is used
     * @param canBeGoogle if connection can be used instead of url
     * */
    fun check(url: String?, canBeGoogle: Boolean = true): Boolean {
        val cm = App.context
        ...
        return if (isConnected(App.context)) {
            ...
            try {
                ...
                val stringUrl = url ?: if (canBeGoogle) {
    ...
    fun canUseInternet(dataEnabledInSettings: Boolean): Boolean {
        /*
        MySettings.withAppContext()
                .getSP()
                .getBoolean(MySettings(App.context).MOBILE_DATA, true)
         */
        return if (dataEnabledInSettings) {
            true
        } else {
            connectedMobileData()
